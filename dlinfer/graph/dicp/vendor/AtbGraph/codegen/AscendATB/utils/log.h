...
class LoggerInitializer {
 ...
 private:
  static void initLogger() {
    ...
    auto logger =
        std::make_shared<spdlog::logger>("dicp", sinks.begin(), sinks.end());
...
#define DICP_LOG(LEVEL)                                                      \
  if (dicp::LoggerInitializer::getCachedLogLevel() <= dicp::LogLevel::LEVEL) \
  dicp::LogMessage(dicp::LogLevel::LEVEL)

#define DICP_LOG_IF(condition, LEVEL)                                        \
  if ((condition) &&                                                         \
      dicp::LoggerInitializer::getCachedLogLevel() <= dicp::LogLevel::LEVEL) \
  dicp::LogMessage(dicp::LogLevel::LEVEL)
