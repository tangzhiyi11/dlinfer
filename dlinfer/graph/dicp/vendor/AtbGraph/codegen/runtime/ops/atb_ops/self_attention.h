...
inline atb::Operation* SelfAttentionOperationCreate(
  ...
  if (paramJson.contains("calcType")) {
    ...
    param.calcType =
        static_cast<atb::infer::SelfAttentionParam::CalcType>(value);
  }
  if (paramJson.contains("kernelType")) {
    auto value = paramJson["kernelType"].get<int32_t>();
    param.kernelType =
        static_cast<atb::infer::SelfAttentionParam::KernelType>(value);
  }
  if (paramJson.contains("clampType")) {
    auto value = paramJson["clampType"].get<int32_t>();
    param.clampType =
        static_cast<atb::infer::SelfAttentionParam::ClampType>(value);
  }
  ...
  if (paramJson.contains("maskType")) {
    ...
    param.maskType =
        static_cast<atb::infer::SelfAttentionParam::MaskType>(value);
  }
  DICP_LOG(INFO) << "SelfAttentionParam: headNum: " << param.headNum
                 << " kvHeadNum: " << param.kvHeadNum
                 << " calcType: " << param.calcType
                 << " kernelType: " << param.kernelType
                 << " clampType: " << param.clampType
                 << " qkScale: " << param.qkScale << " qScale: " << param.qScale
                 << " isTriuMask: " << param.isTriuMask
