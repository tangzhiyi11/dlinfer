...
inline atb::Operation* PagedAttentionOperationCreate(
  ...
  if (paramJson.contains("maskType")) {
    ...
    param.maskType =
        static_cast<atb::infer::PagedAttentionParam::MaskType>(value);
  }
  DICP_LOG(INFO) << "PagedAttentionParam: headNum" << param.headNum
                 << " kvHeadNum: " << param.kvHeadNum
                 << " qkScale: " << param.qkScale
