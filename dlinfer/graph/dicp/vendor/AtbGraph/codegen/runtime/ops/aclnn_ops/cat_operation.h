...
class AclNnCatOperation : public AclNnOperation {
 public:
  explicit AclNnCatOperation(const std::string& name, int32_t inputNum,
                             int32_t concatDim);
 ...
 private:
  ...
  int CreateAclTensors(const atb::VariantPack& variantPack,
                       AclNnTask& task) override;
  int CallAclGetWorkspace(AclNnTask& task, uint64_t& workspaceSize) override;
  int CallAclExecute(uint8_t* workspace, uint64_t workspaceSize,
                     aclOpExecutor* aclExecutor, aclrtStream stream) override;
  AclNnTensor CreateTensor(atb::Tensor atbTensor);
};

inline atb::Operation* AclNnCatOperationCreate(
  ...
  DICP_LOG(INFO) << "AclNnCatOperation: name: " << opName
                 << " inputNum:" << inputNum << " concatDim:" << concatDim;
